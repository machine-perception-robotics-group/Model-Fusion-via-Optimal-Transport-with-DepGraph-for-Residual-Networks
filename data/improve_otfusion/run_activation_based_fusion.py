...
from src.fusion.fusion_act import main_activation_based as main_fusion
...
def run_fusion(result_root_dir, checkpoint_dir, dataset_name, model_name, epoch, use_bn=True):
    ...
    for model_a_filepath in model_filepath_list_a:
        for model_b_filepath in model_filepath_list_b:
            ...
            # バッチ正規化層の統計量再計算なし
            result_dir = join(result_root_dir, dataset_name, model_name + "_wo_recalc_batchnorm_stats", str(cnt))
            main_fusion(
                model_name, dataset_name, model_a_filepath, model_b_filepath, result_dir, recalc_batchnorm_stats=False
            )

            cnt += 1


result_root_dir = "results/activation_based"
...
main_fusion(model_name, dataset_name, model_a_filepath, model_b_filepath, result_dir, recalc_batchnorm_stats=False)
